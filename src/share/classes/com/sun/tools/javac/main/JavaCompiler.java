...
public class JavaCompiler implements ClassReader.SourceCompleter {
    ...
    public static Map<ClassSymbol, JCClassDecl> classSymbolInitMap //map class symbol to its definition
    					= new HashMap<ClassSymbol, JCClassDecl>();
    /** The type eraser.
     */
...
//      System.err.println("completing " + c);//DEBUG
    ...
    private void compile2() {
        try {
            ...
            case BY_TODO:
            	if ( options.isSet(DELAYALLOC)){
            		//pass1
            		for (int i = 0; i < todo.size(); i++){
                		todo.setContent(i, attribute(todo.getContent(i)));
                	}
            		//System.out.println("AFTER ATTRIBUTE: todo=" + todo);
            		//pass2
            		for (int i = 0; i < todo.size(); i++) {
            			generate(desugar(delay(flow(todo.getContent(i)))));
            		}
            		//System.out.println("FINALLy todo=" + todo);
            	} else {
            		while (! todo.isEmpty()){
   ...
   public List<JCCompilationUnit> parseFiles(Iterable<JavaFileObject> fileObjects) {
    ...
    protected void flow(Env<AttrContext> env, Queue<Env<AttrContext>> results) {
     ...
     /**
     * Perform delay allocation nanalysis checks on parse trees.
     * @returns the list of parse trees
     */
    public Queue<Env<AttrContext>> delay(Queue<Env<AttrContext>> envs) {
        ListBuffer<Env<AttrContext>> results = lb();
        for (Env<AttrContext> env: envs) {
            delay(env, results);
        }
        return stopIfError(CompileState.DELAY, results);
    }

    /**
     * Perform dataflow checks on an attributed parse tree.
     */
    public Queue<Env<AttrContext>> delay(Env<AttrContext> env) {
        ListBuffer<Env<AttrContext>> results = lb();
        delay(env, results);
        return stopIfError(CompileState.DELAY, results);
    }
     ...
     * Perform dataflow checks on an attributed parse tree.
     */
    protected void delay(Env<AttrContext> env, Queue<Env<AttrContext>> results) {
        try {
            if (shouldStop(CompileState.DELAY))
                return;

            if (relax || compileStates.isDone(env, CompileState.DELAY)) {
                results.add(env);
                return;
            }

            if (verboseCompilePolicy)
                printNote("[delay " + env.enclClass.sym + "]");
            JavaFileObject prev = log.useSource(
                                                env.enclClass.sym.sourcefile != null ?
                                                env.enclClass.sym.sourcefile :
                                                env.toplevel.sourcefile);
            try {
                make.at(Position.FIRSTPOS);
                TreeMaker localMake = make.forToplevel(env.toplevel);
                delay.analyzeTree(env, localMake);
                compileStates.put(env, CompileState.DELAY);
